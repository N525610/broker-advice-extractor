...
def extract_fields(text):
    ...
    buyer_match = re.search(r"(ALLIED PINNACLE.*?)ABN:\s*(\d{11})", text, re.IGNORECASE | re.DOTALL)
    seller_match = re.search(r"(CARGILL AUSTRALIA LTD.*?)ABN:\s*(\d{11})", text, re.IGNORECASE | re.DOTALL)

    buyer_name = buyer_match.group(1).split('\n')[0].strip() if buyer_match else ""
    buyer_abn = buyer_match.group(2).strip() if buyer_match else ""
    seller_name = seller_match.group(1).split('\n')[0].strip() if seller_match else ""
    seller_abn = seller_match.group(2).strip() if seller_match else ""
